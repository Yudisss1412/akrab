...
@section('title', 'Edit Diskon Produk - ' . ($promotionData['name'] ?? 'Diskon Produk'))
    ...
    <div class="container-fluid">
      ...
      <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
          ...
          <h1 class="page-title mb-0">Edit Diskon Produk: {{ $promotionData['name'] ?? 'Diskon Produk' }}</h1>
        </div>
      </div>
      
      <!-- Formulir Diskon -->
      <form id="editDiscountForm" action="{{ route('penjual.promosi.discount.update', ['id' => $promotionData['id']]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <input type="text" id="discountName" name="name" class="form-control" value="{{ old('name', $promotionData['name'] ?? '') }}" placeholder="Contoh: Diskon Akhir Tahun">
          </div>
        </div>
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <div class="radio-group">
              <div class="radio-option">
                <input type="radio" id="typePercentage" name="type" value="percentage" {{ old('type', $promotionData['discount_type'] ?? '') === 'percentage' ? 'checked' : '' }}>
                <label for="typePercentage">Persentase (%)</label>
              </div>
              <div class="radio-option">
                <input type="radio" id="typeFixed" name="type" value="fixed_amount" {{ old('type', $promotionData['discount_type'] ?? '') === 'fixed_amount' ? 'checked' : '' }}>
          ...
          <div class="form-group">
            ...
            <input type="number" id="discountValue" name="discount_value" class="form-control" value="{{ old('discount_value', $promotionData['discount_value'] ?? '') }}" placeholder="Masukkan besar diskon">
          </div>
        </div>
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            <label class="form-label">Produk yang Didiskon</label>
            <div class="selected-products" id="selectedProductsContainer">
              <div class="selected-product-item">
                <span>{{ $promotionData['product']->name ?? 'Produk Tidak Ditemukan' }}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="datetime-local" id="startDate" name="start_date" class="form-control" value="{{ old('start_date', $promotionData['start_date'] ?? '') }}">
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label for="endDate" class="form-label">Tanggal & Waktu Selesai</label>
                <input type="datetime-local" id="endDate" name="end_date" class="form-control" value="{{ old('end_date', $promotionData['end_date'] ?? '') }}">
              </div>
            </div>
          </div>
        </div>
