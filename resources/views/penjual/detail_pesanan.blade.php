                ...
                @if($order->status === 'pending')
                  @if(isset($order->payment) && $order->payment && in_array($order->payment->payment_method, ['midtrans', 'bank_transfer', 'e_wallet']))
                    @if($order->payment->payment_status === 'success')
                      <span class="payment-method-info" style="color: #059669; font-size: 0.9em; display: inline-block; padding: 8px 12px; background-color: #d1fae5; border-radius: 6px;">✓ Pembayaran Lunas (Midtrans)</span>
                    @elseif($order->payment->payment_status === 'pending_verification' || $order->payment->payment_status === 'pending')
                      <span class="payment-method-info" style="color: #d97706; font-size: 0.9em; display: inline-block; padding: 8px 12px; background-color: #fef3c7; border-radius: 6px;">⏳ Menunggu Pembayaran Midtrans</span>
                    ...
                    @elseif($order->payment->payment_status === 'failed')
                      <span class="payment-method-info" style="color: #dc2626; font-size: 0.9em; display: inline-block; padding: 8px 12px; background-color: #fee2e2; border-radius: 6px;">✗ Pembayaran Gagal</span>
                    @else
