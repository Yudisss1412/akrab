...
@section('title', 'Edit Voucher Toko - ' . ($promotion->name ?? 'Voucher Toko'))
    ...
    <div class="container-fluid">
      ...
      <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
          ...
          <h1 class="page-title mb-0">Edit Voucher Toko: {{ $promotion->name ?? 'Voucher Toko' }}</h1>
        </div>
      </div>
      
      <!-- Formulir Voucher -->
      <form id="editVoucherForm" action="{{ route('penjual.promosi.update', ['id' => $promotion->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <input type="text" id="voucherName" name="name" class="form-control" value="{{ old('name', $promotion->name ?? '') }}" placeholder="Contoh: Voucher Hemat Akhir Tahun">
          </div>
          
          <div class="form-group">
            <label for="voucherCode" class="form-label">Kode Voucher</label>
            <div class="input-group">
              <input type="text" id="voucherCode" name="code" class="form-control" value="{{ old('code', $promotion->code ?? '') }}" placeholder="Contoh: HEMAT2024">
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <div class="radio-group">
              <div class="radio-option">
                <input type="radio" id="typePercentage" name="type" value="percentage" {{ old('type', $promotion->type ?? '') === 'percentage' ? 'checked' : '' }}>
                <label for="typePercentage">Persentase (%)</label>
              </div>
              <div class="radio-option">
                <input type="radio" id="typeFixed" name="type" value="fixed_amount" {{ old('type', $promotion->type ?? '') === 'fixed_amount' ? 'checked' : '' }}>
                <label for="typeFixed">Potongan Harga Tetap (Rp)</label>
              </div>
              <div class="radio-option">
                <input type="radio" id="typeFreeShipping" name="type" value="free_shipping" {{ old('type', $promotion->type ?? '') === 'free_shipping' ? 'checked' : '' }}>
          ...
          <div class="form-group">
            ...
            <input type="number" id="discountValue" name="discount_value" class="form-control" value="{{ old('discount_value', $promotion->discount_value ?? '') }}" placeholder="Masukkan besar diskon">
          </div>
          ...
          <div class="form-group max-discount-group" id="maxDiscountGroup">
            ...
            <input type="number" id="maxDiscount" name="max_discount_amount" class="form-control" value="{{ old('max_discount_amount', $promotion->max_discount_amount ?? '') }}" placeholder="Masukkan maksimum potongan">
          </div>
        </div>
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <input type="number" id="minPurchase" name="min_order_amount" class="form-control" value="{{ old('min_order_amount', $promotion->min_order_amount ?? 0) }}" placeholder="Rp 0">
          </div>
          
          <div class="form-group">
            <label for="quota" class="form-label">Kuota Penggunaan</label>
            <input type="number" id="quota" name="usage_limit" class="form-control" value="{{ old('usage_limit', $promotion->usage_limit ?? 0) }}" placeholder="Jumlah total voucher yang tersedia (0 untuk unlimited)">
          </div>
        </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="datetime-local" id="startDate" name="start_date" class="form-control" value="{{ old('start_date', $promotion->start_date ? $promotion->start_date->format('Y-m-d\\TH:i') : '') }}">
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label for="endDate" class="form-label">Tanggal & Waktu Selesai</label>
                <input type="datetime-local" id="endDate" name="end_date" class="form-control" value="{{ old('end_date', $promotion->end_date ? $promotion->end_date->format('Y-m-d\\TH:i') : '') }}">
              </div>
            </div>
          </div>
        </div>
